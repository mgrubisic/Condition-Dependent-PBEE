...
Recent studies \cite{Ma2012}, \cite{Meda2014} and \citep{Yang2016} have been developed to assess the force-displacement relationships in cantilever RC Columns. These columns were subjected to Quasi-Static Loading Protocol, the concrete columns were subjected to accelerated corrosion to obtain different Corrosion Levels ($CL$), the range of $CL$ for these studies correspond to $CL=0\%\-20\%$. In these studies the accelerated corrosion was performed via an electrochemical process directly applied to the reinforcing steel as shown in Figure.

The resulting force displacement of these experiments is shown in Figure as it can be seen there is a reduction not only on the strength of the system but also on the displacement capacity. 

As stated in the previous section the mechanical properties of steel are affected by corrosion, in the previous studies \cite{Meda2014} the authors performed tension tests on corroded reinforcing steel, how ever these studies dis not use the proper methods since as it's known reinforcing steel generates a protective film due to the alkaline environment of the concrete, these tests did not consider it and accelerated corrosion using NaCl while the chloride attack in concrete is , therefore the results might not be representative of what actually occurs in corroded RC columns, therefore an experimental campaign is proposed that would shed a light into the properties of corroded reinforcing steel inside concrete.
